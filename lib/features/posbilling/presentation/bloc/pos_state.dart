...
class PosCartState extends PosState {
  ...
  double get rawSubtotal => MoneyUtil.roundMoney(cartItems.fold(0.0, (sum, item) => sum + item.rawSubtotal));
  
  /// Sum of all product-level discounts
  double get productDiscounts => MoneyUtil.roundMoney(cartItems.fold(0.0, (sum, item) => sum + item.discountAmount));
  
  /// Subtotal after item-level discounts but before overall discount
  double get subtotal => MoneyUtil.roundMoney(cartItems.fold(0.0, (sum, item) => sum + item.totalPrice));
  
  /// Total discounts (Product-wise discounts + Overall discount)
  double get totalDiscount => MoneyUtil.roundMoney(productDiscounts + discountAmount);
  
  double get vatAmount => 0.0; // Can be configured
  
  /// Final Net Total to be paid by customer
  double get netTotal => MoneyUtil.roundMoney((subtotal - discountAmount + vatAmount).clamp(0.0, double.infinity));
